...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj, $type , $timeOut = 15)
	{
		...
		$time_stamp = time()+rand(10000,90000);
		$pack	= 'Sign=WXPay';
		//输出参数列表
		$prePayParams =array();
		...
		if($type=='wxmini'){
			$prePayParams['signs']	=MD5('appId='.$result['appid'].'&nonceStr='.$result['nonce_str'].'&package=prepay_id='.$result['prepay_id'].'&signType=MD5&timeStamp='.$time_stamp.'&key='.(WxPayConfig::KEY)); 
		}
