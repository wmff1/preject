...
class Service
{
    ...
    * @param string $method
    */
    public static function queryOrder($trade_no, $paytype = 'alipay'){

        $config = Service::getConfig($paytype);
        if($paytype == 'alipay'){
            $pay = Pay::alipay($config);
            $pay_data = $pay->find(['out_trade_no' => $trade_no]);
            if($pay_data->code == 10000 && $pay_data->trade_status == 'TRADE_SUCCESS'){
                return true;
            }
        }else{
            $pay = Pay::wechat($config);
            $pay_data = $pay->find(['out_trade_no' => $trade_no, 'type' => 'app']);
