...
use app\common\model\Product\Activity as ActivityModel;
use app\admin\library\traits\ActivityCache;
use app\common\model\Product\ActivitySkuPrice;
use app\common\model\Product\Product;
use app\common\model\Product\Sku;
use app\common\model\Product\SkuPrice;
use think\Db;
...
class Activity extends Base
{
    ...
    public function _initialize()
    {
        ...
        $this->assignconfig("hasRedis", $this->hasRedis());
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if ($goodsIdsArr) {
                // 查询商品
                $goods = Product::where('id', 'in', $goodsIdsArr)->select();
    ...
    {
       //获取活动类型和状态
       $activity_type = ActivityModel::getTypeList();
       $activity_status = ActivityModel::getStatusList();
    ...
    public function sku()
    {
        $id = $this->request->get('id', 0); // 45
        $activity_id = $this->request->get('activity_id', 0); // ''
        $activity_type = $this->request->get('activity_type', ''); // seckill
        $type = $this->request->get('type', 0); // add
        $activitytime = $this->request->get('activitytime', '') ? $this->request->get('activitytime', '') : ''; // ''
        $activitytime = array_filter(explode(' - ', $activitytime));// empty
        ...
        // 商品规格
        $skuList = Sku::with(['children' => function ($query) use ($id) {
            $query->where('goods_id', $id);
        }])->where(['pid' => 0, 'goods_id' => $id])->select();

        // 获取规格
        $skuPrice = SkuPrice::with(['activitySkuPrice' => function ($query) use ($activity_id) {
    ...
    protected function createOrUpdateSku($goodsList, $activity_id, $type = 'all')
    {
        ...
        ActivitySkuPrice::where(['activity_id' => $activity_id])->update(['status' => 'down']);
        $list = [];
    ...
    { 
        ...
        $adminIds = $this->getDataLimitAdminIds(); // null
        ...
        foreach ($goods_ids_array as $k => $g) {
            $goods[$k] = Product::field('id,name,thumbs as image')->where('id', $g)->find();
            $goods[$k]['actSkuPrice'] = json_encode(ActivitySkuPrice::all(['goods_id' => $g, 'activity_id' => $ids]));
            $goods[$k]['opt'] = 1;
            $goodsList[] = $goods[$k];
        }
